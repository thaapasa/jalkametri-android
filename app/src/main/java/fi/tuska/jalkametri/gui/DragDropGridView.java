...
public class DragDropGridView extends GridView {
    ...
    public DragDropGridView(Context context, AttributeSet attrs) {
        ...
        setItemDimensions(10, 10);
    }

    private void setItemDimensions(int width, int height) {
        mItemHeightNormal = height;
        mItemHeightHalf = height / 2;
        mItemHeightExpanded = (int) (height * 1.2);

        mItemWidthNormal = width;
        mItemWidthHalf = width / 2;
        mItemWidthExpanded = (int) (width * 1.2);
    }

    @Override
    public void setColumnWidth(int width) {
        super.setColumnWidth(width);
        mItemWidthNormal = width;
        mItemWidthExpanded = (int) (width * 1.2);
        mItemWidthHalf = width / 2;
    }
    ...
    private int getItemForPosition(int x, int y) {
        ...
        int adjustedx = x - mDragPointX - mItemWidthHalf;
        ...
        for (int i = 0;; i++) {
            ...
            if (v == null) {
                try {
                    ...
                    // layoutChildren throws this sometimes, presumably
                    // because we're in the process of being torn down but are
                    // still getting touch events
                }
                if (v == null) {
                    return;
                }
     ...
     * if the dragged listitem is still on screen, make it as small as
     * possible and expand the item below the insert point. If the dragged
     * item is not on screen, only expand the item below the current
     * insertpoint.
     */
    private void doExpansion() {
        ...
        // Currently assume that there are no headers in this grid view.
        // Fix this if required.
        final int numheaders = 0;
        ...
        for (int i = 0;; i++) {
            ...
            if (mDragPos < numheaders && i == numheaders) {
                // dragging on top of the header item, so adjust the item
                // below
                // instead
                if (vv.equals(first)) {
    ...
    private void startDragging(Bitmap bm, int x, int y) {
        ...
        if (firstChild != null) {
            setItemDimensions(firstChild.getWidth(), firstChild.getHeight());
        }
