...
public class FAQCalendar extends GregorianCalendar {
    ...
    private static final long serialVersionUID = 1777954114360700509L;

    /**
     * All minutes have this many milliseconds except the last minute of the
     * day on a day defined with a leap second.
     */
    ...
    public FAQCalendar(Calendar cal) {
        super(cal.get(Calendar.YEAR), cal.get(Calendar.MONTH), cal.get(Calendar.DAY_OF_MONTH),
            cal.get(Calendar.HOUR_OF_DAY), cal.get(Calendar.MINUTE), cal.get(Calendar.SECOND));
    }

    public FAQCalendar(Date date, Context context) {
        this(new TimeUtil(context).getCalendar(date));
    }
     ...
     * midnight LOCAL TIME.
     * @see http://tycho.usno.navy.mil/mjd.html for more information about
     * local C-JDN
     */
    ...
    /**
     * Finds the number of days from this date to the given end date. Later
     * end dates result in positive values. Note this is not the same as
