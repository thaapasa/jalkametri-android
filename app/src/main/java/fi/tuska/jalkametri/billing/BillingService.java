...
import fi.tuska.jalkametri.billing.Security.VerifiedPurchase;
...
public class BillingService extends Service implements ServiceConnection {
    ...
    private static final String BILLING_SERVICE_BIND_ADDRESS = "com.android.vending.billing.MarketBillingService.BIND";
    ...
    /**
     * The list of requests that we have sent to Android Market but for which
     * we have not yet received a response code. The HashMap is indexed by the
     ...
     * derived class overrides the run() method to call the appropriate
     * service interface. If we are already connected to the
     * MarketBillingService, then we call the run() method directly.
     * Otherwise, we bind to the service and save the request on a queue to be
     * run later when the service is connected.
     */
    public abstract class BillingRequest {
        ...
        public boolean runIfConnected() {
            ...
            if (mService != null) {
                try {
                    mRequestId = run();
                    LogUtil.d(TAG, "request id: " + mRequestId);
                    if (mRequestId >= 0) {
        ...
        protected Bundle makeRequestBundle(String method) {
            ...
            request.putString(Common.BILLING_REQUEST_METHOD, method);
            request.putInt(Common.BILLING_REQUEST_API_VERSION, 1);
            request.putString(Common.BILLING_REQUEST_PACKAGE_NAME, Common.BILLING_PACKAGE_NAME);
            return request;
        }

        protected void logResponseCode(String method, Bundle response) {
            ResponseCode responseCode = Common.ResponseCode.valueOf(response
                .getInt(Common.BILLING_RESPONSE_RESPONSE_CODE));
            LogUtil.d(TAG, "%s received %s", method, responseCode.toString());
        }
    }
    ...
    public class CheckBillingSupported extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            ...
            int responseCode = response.getInt(Common.BILLING_RESPONSE_RESPONSE_CODE);
            LogUtil.i(TAG,
            ...
            return Common.BILLING_RESPONSE_INVALID_REQUEST_ID;
        }
    ...
    public class RequestPurchase extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            ...
            request.putString(Common.BILLING_REQUEST_ITEM_ID, mProductId);
            // Note that the developer payload is optional.
            if (mDeveloperPayload != null) {
                request.putString(Common.BILLING_REQUEST_DEVELOPER_PAYLOAD, mDeveloperPayload);
            }
            Bundle response = mService.sendBillingRequest(request);
            PendingIntent pendingIntent = response
                .getParcelable(Common.BILLING_RESPONSE_PURCHASE_INTENT);
            ...
            return response.getLong(Common.BILLING_RESPONSE_REQUEST_ID,
                Common.BILLING_RESPONSE_INVALID_REQUEST_ID);
        }

        @Override
        protected void responseCodeReceived(ResponseCode responseCode) {
            LogUtil.i(TAG, "Response for RequestPurchase: %s", responseCode);
    ...
    public class ConfirmNotifications extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            ...
            request.putStringArray(Common.BILLING_REQUEST_NOTIFY_IDS, mNotifyIds);
            Bundle response = mService.sendBillingRequest(request);
    ...
    public class GetPurchaseInformation extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            ...
            request.putLong(Common.BILLING_REQUEST_NONCE, mNonce);
            request.putStringArray(Common.BILLING_REQUEST_NOTIFY_IDS, mNotifyIds);
            Bundle response = mService.sendBillingRequest(request);
    ...
    public IBinder onBind(Intent intent) {
        // Return null because this is not supported
        return null;
    }
    ...
    public void handleCommand(Intent intent, int startId) {
        // Intent may be null in some situation; at least it has been null
        // based on an exception report in Android Market on 2011-05-05
        if (intent == null)
            return;
        String action = intent.getAction();
        LogUtil.i(TAG, "handleCommand() action: " + action);

        if (Common.ACTION_CONFIRM_NOTIFICATION.equals(action)) {
            String[] notifyIds = intent.getStringArrayExtra(Common.NOTIFICATION_ID);
            confirmNotifications(startId, notifyIds);
        } else if (Common.ACTION_GET_PURCHASE_INFORMATION.equals(action)) {
            String notifyId = intent.getStringExtra(Common.NOTIFICATION_ID);
            getPurchaseInformation(startId, new String[] { notifyId });
        } else if (Common.ACTION_PURCHASE_STATE_CHANGED.equals(action)) {
            String signedData = intent.getStringExtra(Common.INAPP_SIGNED_DATA);
            String signature = intent.getStringExtra(Common.INAPP_SIGNATURE);
            purchaseStateChanged(startId, signedData, signature);
        } else if (Common.ACTION_RESPONSE_CODE.equals(action)) {
            long requestId = intent.getLongExtra(Common.INAPP_REQUEST_ID, -1);
            int responseCodeIndex = intent.getIntExtra(Common.INAPP_RESPONSE_CODE,
    ...
    private boolean bindToMarketBillingService() {
        try {
            ...
            if (bindResult) {
                ...
                LogUtil.e(TAG, "Could not bind to service.");
            }
        } catch (SecurityException e) {
            LogUtil.e(TAG, "Security exception: %s", e);
        }
        return false;
    }
    ...
    public boolean checkBillingSupported() {
        LogUtil.i(TAG, "Checking if billing is supported");
        return new CheckBillingSupported().runRequest();
    }

    /**
     * Requests that the license item be offered to the user for purchase.
     * When the purchase succeeds (or is canceled) the {@link BillingReceiver}
     * receives an intent with the action {@link Consts#ACTION_NOTIFY}.
     * Returns false if there was an error trying to connect to Android
     * Market.
     */
    public boolean purchaseLicense() {
        return new RequestPurchase(PrivateData.LICENSE_PRODUCT_ID,
            PrivateData.LICENSE_PURCHASE_PAYLOAD).runRequest();
    }
     ...
     * request. The server responds with the purchase information, encoded as
     * a JSON string, and sends that to the {@link BillingReceiver} in an
    ...
    private void purchaseStateChanged(int startId, String signedData, String signature) {
        LogUtil.d(TAG, "Purchase state changed: %s, %s", signedData, signature);
     ...
     * a request that we made. This is used for reporting various errors and
     * for acknowledging that an order was sent to the server. This is NOT
     * used for any purchase state changes. All purchase state changes are
    ...
    private void runPendingRequests() {
        ...
        // If we get here then all the requests ran successfully. If
        // maxStartId
        // is not -1, then one of the requests started the service, so we can
        // stop it now.
        if (maxStartId >= 0) {
            LogUtil.i(TAG, "stopping service, startId: " + maxStartId);
            stopSelf(maxStartId);
        }
    }
    ...
    public void onServiceConnected(ComponentName name, IBinder service) {
        LogUtil.i(TAG, "MarketBillingService connected.");
        mService = IMarketBillingService.Stub.asInterface(service);
        if (mService == null) {
            LogUtil.e(TAG, "Billing service is null");
        }
        runPendingRequests();
    }
