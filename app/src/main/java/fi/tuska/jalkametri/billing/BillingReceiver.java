...
public class BillingReceiver extends BroadcastReceiver {
     ...
     * Market. The {@link BillingService} also reports state changes back to
     * the application through the {@link ResponseHandler}.
     */
    @Override
    public void onReceive(Context context, Intent intent) {
        String action = intent.getAction();
        if (Common.ACTION_PURCHASE_STATE_CHANGED.equals(action)) {
        ...
        } else if (Common.ACTION_NOTIFY.equals(action)) {
            // Notification message
            String notifyId = intent.getStringExtra(Common.NOTIFICATION_ID);
            LogUtil.i(TAG, "notifyId: " + notifyId);
    ...
    private void purchaseStateChanged(Context context, String signedData, String signature) {
        Intent intent = new Intent(Common.ACTION_PURCHASE_STATE_CHANGED);
        intent.setClass(context, BillingService.class);
        intent.putExtra(Common.INAPP_SIGNED_DATA, signedData);
        intent.putExtra(Common.INAPP_SIGNATURE, signature);
        LogUtil.d(TAG, "Starting service for intent %s", intent);
    ...
    private void notify(Context context, String notifyId) {
        Intent intent = new Intent(Common.ACTION_GET_PURCHASE_INFORMATION);
        intent.setClass(context, BillingService.class);
        intent.putExtra(Common.NOTIFICATION_ID, notifyId);
        context.startService(intent);
    }
    ...
    private void checkResponseCode(Context context, long requestId, int responseCodeIndex) {
        Intent intent = new Intent(Common.ACTION_RESPONSE_CODE);
        intent.setClass(context, BillingService.class);
        intent.putExtra(Common.INAPP_REQUEST_ID, requestId);
        intent.putExtra(Common.INAPP_RESPONSE_CODE, responseCodeIndex);
        context.startService(intent);
    }
}
