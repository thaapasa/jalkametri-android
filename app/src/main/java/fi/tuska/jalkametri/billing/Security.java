...
import fi.tuska.jalkametri.util.Base64;
import fi.tuska.jalkametri.util.Base64DecoderException;
import fi.tuska.jalkametri.util.LogUtil;

/**
 * Security-related methods. For a secure implementation, all of this code
 * should be implemented on a server that communicates with the application on
 * the device. For the sake of simplicity and clarity of this example, this
 * code is included here and is executed on the device. If you must verify the
 * purchases on the phone, you should obfuscate this code to make it harder
 * for an attacker to replace the code with stubs that treat all purchases as
 * verified.
 */
public class Security {
     ...
     * killed and lose this list of nonces, it is not fatal. Android Market
     * will send us a new "notify" message and we will re-generate a new
     * nonce. This has to be "static" so that the {@link BillingReceiver} can
     ...
     * a nonce (number used once) that we generated and that was signed (as
     * part of the whole data string) with a private key. The data also
     * contains the {@link PurchaseState} and product ID of the purchase. In
    ...
    public static ArrayList<VerifiedPurchase> verifyPurchase(String signedData, String signature) {
        ...
        LogUtil.i(TAG, "SignedData: %s", signedData);
        ...
        if (!TextUtils.isEmpty(signature)) {
             ...
             * perform.
             */
            String base64EncodedPublicKey = PrivateData.ACCOUNT_PUBLIC_KEY;
            PublicKey key = Security.generatePublicKey(base64EncodedPublicKey);
            verified = Security.verify(key, signedData, signature);
            if (!verified) {
                LogUtil.w(TAG, "signature does not match data.");
                return null;
            }
        }
        ...
        if (!Security.isNonceKnown(nonce)) {
            LogUtil.w(TAG, "Nonce not found: " + nonce);
            return null;
        }
        ...
        try {
            ...
            LogUtil.e(TAG, "JSON exception: ", e);
            return null;
        }
        removeNonce(nonce);
        return purchases;
