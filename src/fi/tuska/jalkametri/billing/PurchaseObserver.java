...
/**
 * An interface for observing changes related to purchases. The main
 * application extends this class and registers an instance of that derived
 * class with {@link ResponseHandler}. The main application implements the
 * callbacks {@link #onBillingSupported(boolean)} and
...
public abstract class PurchaseObserver {
    ...
    /**
     * This is the callback that is invoked when the license state changes. It
     * is the callback invoked in response to calling
     ...
     * {@link #onPurchaseStateChange(PurchaseState, String, int, long)}. This
     * is used for reporting various errors, or if the user backed out and
     * didn't purchase the item. The possible response codes are: RESULT_OK
     * means that the order was sent successfully to the server. The
     ...
     * connect to the Android Market server (for example if the data
     * connection is down). RESULT_BILLING_UNAVAILABLE means that in-app
     * billing is not supported yet. RESULT_ITEM_UNAVAILABLE means that the
     * item this app offered for sale does not exist (or is not published) in
     * the server-side catalog. RESULT_ERROR is used for any other errors
     ...
     * a RestoreTransactions request that we made. A response code of
    ...
    void startBuyPageActivity(PendingIntent pendingIntent, Intent intent) {
        if (mStartIntentSender != null) {
            ...
            try {
                ...
                LogUtil.e(TAG, "error starting activity", e);
            }
        } else {
            // This is on Android version 1.6. The in-app buy page activity
            // must be on its own separate activity stack instead of on the
     ...
     */
    void postLicenseStateChanged(final boolean hasLicense) {
        mHandler.post(new Runnable() {
            public void run() {
                onLicenseStateChanged(hasLicense);
            }
        });
    }
}
